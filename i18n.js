...
b.textContent=chrome.i18n.getMessage(d)},'i18n-values':function(b,d){
for(var f=d.replace(/\s/g,'').split(/;/),e=0;e<f.length;e++){
var c=f[e].match(/^([^:]+):(.+)$/);if(c){var a=c[1];
c=chrome.i18n.getMessage(c[2]);if(a.charAt(0)=='.'){a=a.slice(1).split('.');
for(var g=b;g&&a.length>1;)g=g[a.shift()];if(g){g[a]=c;a=='innerHTML'&&i(b)}}
